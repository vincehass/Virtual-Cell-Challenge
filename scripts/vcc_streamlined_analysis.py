#!/usr/bin/env python3
"""
🧬 STREAMLINED Virtual Cell Challenge Analysis
Using existing processed data for fast development and analysis.
Optimized for quick results without external drive complexity.
"""
...
def comprehensive_analysis(adata, dataset_name, output_dirs):
    ...
    print(f"  📊 Mean genes/cell: {np.mean(total_expr):.1f}")
...
def main():
    ...
    print("🧬 Virtual Cell Challenge - Streamlined Analysis")
    print("=" * 60)
    ...
    wandb_run = wandb.init(
        ...
        name=f"streamlined-analysis-{selected_dataset}-{datetime.now().strftime('%H%M%S')}",
        config={
            "dataset_name": selected_dataset,
            "dataset_size_gb": dataset_info['size_gb'],
            "analysis_type": "streamlined",
            "memory_optimized": True
        },
        tags=["streamlined", "processed-data", selected_dataset]
    )
    ...
    try:
        ...
        # Log to W&B
        wandb.log(results['dataset_info'])
        if 'perturbation_analysis' in results:
            wandb.log(results['perturbation_analysis'])
        if 'expression_analysis' in results:
            wandb.log(results['expression_analysis'])
        
        # Create visualizations
        figure_file = create_comprehensive_visualizations(adata, selected_dataset, results, output_dirs)
        
        # Log figure to W&B
        wandb.log({"comprehensive_analysis": wandb.Image(str(figure_file))})
