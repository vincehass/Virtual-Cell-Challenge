#!/usr/bin/env python3
"""
🧬 MEMORY-OPTIMIZED Virtual Cell Challenge Pipeline with W&B Logging

Optimized for large datasets (200K+ cells) to avoid memory overflow.
Uses chunk processing and memory-efficient operations.
"""
...
def setup_wandb_experiment():
    ...
    config = {
        ...
        "dataset_name": "VCC_Training_Real_MemoryOptimized",
        ...
        "pipeline_version": "1.1.0_memory_optimized",
        ...
        "sample_for_analysis": 20000,  # Sample for analysis to save memory
        ...
        # Hardware
        "compute_environment": "CPU_Memory_Optimized",
        "python_version": "3.9",
    }
    ...
    run = wandb.init(
        ...
        name="vcc-memory-optimized-pipeline",
        notes="Memory-optimized VCC pipeline for large datasets (221K cells)",
        tags=["data-analysis", "preprocessing", "real-data", "vcc", "memory-optimized"],
        config=config
    )
...
def step1_data_loading_efficient(config):
    ...
    # Top perturbations
    top_perts = perturbation_counts.head(15)  # Fewer for readability
    ...
    ax1.set_title('Top 15 Perturbations')
    
    # Batch distribution
    ax2.hist(batch_counts.values, bins=min(20, len(batch_counts)), alpha=0.7, edgecolor='black')
...
def step2_sampled_analysis(adata, config):
    ...
    for pert in perturbations.unique():
        ...
        n_sample = min(100, len(pert_indices))  # Max 100 cells per perturbation
        if n_sample > 0:
    ...
    fig.suptitle('VCC Dataset Sample Analysis (Memory Optimized)', fontsize=14, fontweight='bold')
    
    # UMI distribution
    cell_totals = expr_data.sum(axis=1)
    axes[0, 0].hist(cell_totals, bins=30, alpha=0.7, edgecolor='black')
    ...
    axes[0, 1].hist(gene_detection, bins=30, alpha=0.7, edgecolor='black')
    ...
    axes[1, 0].scatter(cell_totals, gene_detection, alpha=0.6, s=2)
    ...
    axes[1, 1].hist(sample_pert_counts.values, bins=15, alpha=0.7, edgecolor='black')
...
def step3_efficient_preprocessing(adata, config):
    ...
    # Create a full copy for processing (this will load into memory)
...
def step4_create_data_splits(adata_processed, config):
    """Step 4: Create training splits without heavy embedding computation."""
    start_time = log_step_start("Data Splits", "Creating training/validation/test splits")
    ...
    # Save splits
    adata_processed[train_mask].write(output_dir / "vcc_train_memory_opt.h5ad")
    adata_processed[val_mask].write(output_dir / "vcc_val_memory_opt.h5ad")
    adata_processed[test_mask].write(output_dir / "vcc_test_memory_opt.h5ad")
    
    # Save full processed dataset
    adata_processed.write(output_dir / "vcc_complete_memory_opt.h5ad")
    ...
    ax.set_title('Data Split Sizes (Memory Optimized)')
    ...
    log_step_complete("Data Splits", start_time, split_metrics)
    return split_metrics

def main():
    """Memory-optimized main pipeline execution."""
    print("🧬 VIRTUAL CELL CHALLENGE - MEMORY-OPTIMIZED PIPELINE")
    print("=" * 60)
    print("📝 Step-by-step analysis with W&B logging")
    print("⚡ Optimized for large datasets (221K cells)")
    print("🎯 Goal: Prepare VCC data efficiently")
    print("=" * 60)
    ...
    try:
        ...
        split_metrics = step4_create_data_splits(adata_processed, config)
        
        # Pipeline completion
        total_time = time.time() - pipeline_start
        
        print(f"\n🎉 MEMORY-OPTIMIZED PIPELINE COMPLETED!")
        ...
        print(f"\n📁 Generated files:")
        print(f"   • data/processed/vcc_complete_memory_opt.h5ad")
        print(f"   • data/processed/vcc_train_memory_opt.h5ad")
        print(f"   • data/processed/vcc_val_memory_opt.h5ad")
        print(f"   • data/processed/vcc_test_memory_opt.h5ad")
